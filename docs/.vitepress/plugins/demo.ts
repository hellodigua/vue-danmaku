  ...
  md.use(mdContainer, 'demo', {
    ...
    render(tokens, idx) {
      ...
      if (tokens[idx].nesting === 1 /* 表示标签正在打开 */) {
        ...
        let source = fs.readFileSync(resolve('docs/examples/', `${sourcePath}.vue`), 'utf-8')
        return `
          <Demo source="${encodeURIComponent(source)}" path="${sourcePath}" description="${encodeURIComponent(
